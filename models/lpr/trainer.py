...
class InputData(object):
  def __init__(self, batch_size, input_shape, file_list_path,
               ...
               stn_alignment=True, use_lbp=False):
    ...
    self.stn_alignment = stn_alignment
  ...
  def input_fn(self):
    ...
    if self.use_lbp:
      lbp, = tf.py_func(compute_lbp, [tf.squeeze(tf.image.rgb_to_grayscale(image), squeeze_dims=[3])], [tf.float32])
      height, width, _ = self.input_shape
      lbp.set_shape([self.batch_size, height, width])
      data = tf.concat(3, [image, tf.expand_dims(lbp, -1)])
...
# Alignment transformation estimation subnetwork
def loc_net(images, global_step=None):
  with slim.arg_scope([slim.fully_connected, slim.conv2d],
                      activation_fn=tf.nn.relu,
                      normalizer_fn=slim.batch_norm,
                      weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
                      weights_regularizer=slim.l2_regularizer(0.0005)):
    param_net_hd = slim.conv2d(images, 32, [5, 5], stride=5, scope='locnet/conv1')
    param_net_ld = slim.avg_pool2d(images, [3, 3], stride=2, scope='locnet/pool')
    param_net_ld = slim.conv2d(param_net_ld, 32, [5, 5], stride=3, scope='locnet/conv2')

    param_net = tf.concat(axis=1, values=[slim.flatten(param_net_ld), slim.flatten(param_net_hd)])
    param_net = slim.dropout(param_net, scope='locnet/drop')
    param_net = slim.fully_connected(param_net, 64, activation_fn=tf.tanh, normalizer_fn=None, scope='locnet/fc1')
    param_net = slim.fully_connected(param_net, 6, activation_fn=tf.tanh, normalizer_fn=None, scope='locnet/fc2')
    param_net = tf.scalar_mul(0.2, param_net)  # restricted transformations
    param_net = tf.reshape(param_net, (-1, 2, 3))

    # curriculum
    if global_step is not None:
      curriculum_rate = tf.clip_by_value(0.0001 * (tf.cast(global_step, tf.float32) - 5000), 0.0, 1.0)
      param_net = tf.scalar_mul(curriculum_rate, param_net)

    return param_net


# Image alignment subnetwork
def align(images, transform_params):
  return apply_stn(images, tf.add(identity_transform(images), transform_params))


# Function for computing LBP features
def compute_lbp(images):
    result = []
    for k in range(images.shape[0]):
        result.append(local_binary_pattern(images[k] / 256.0, 8, 3).astype(np.float32))
