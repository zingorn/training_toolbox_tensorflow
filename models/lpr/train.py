...
def train(config):
  ...
  input_train_data = InputData(batch_size=config.train.batch_size,
                               ...
                               stn_alignment=config.train.stn_alignment,
                               use_lbp=config.train.use_lbp)
  ...
  if config.train.model == '':
    work_path = os.path.join(config.model_dir,'{:%Y_%m_%d_%H_%M}'.format(datetime.datetime.now()))
  else:
    work_path = os.path.join(config.model_dir,'{}'.format(config.train.model))
    if os.path.exists(work_path):
      tf.logging.info('Restore from: ' + work_path)
      saver.restore(session, work_path + '/snapshot{:06d}.ckpt'.format(config.train.start_iter))
      tf.logging.info('Work path is: ' + work_path)
    else:
      os.mkdir(work_path)
